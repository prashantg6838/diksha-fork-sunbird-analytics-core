 ...
 * @author Santhosh
 */
object OCIDispatcher extends HadoopDispatcher with IDispatcher {

  implicit val className = "org.ekstep.analytics.framework.dispatcher.OCIDispatcher"
  ...
  override def dispatch(config: Map[String, AnyRef], events: RDD[String])(implicit sc: SparkContext, fc: FrameworkContext): Unit = {
    ...
    val srcFile = CommonUtil.getOCIFile(bucket, "_tmp/" + key);
    val destFile = CommonUtil.getOCIFile(bucket, key);
